...
#### `useNavigationGuard.test.ts` (19개)
...
| allowNavigation | 호출 후 popstate 발생해도 다이얼로그 열리지 않음 |
...
| 152-154 | `onCancel`에서 `isLeaving=true` 분기 | 이미 확인된 내비게이션 중 cancel 호출 — 실제 UX 상 발생 빈도 낮음 |
