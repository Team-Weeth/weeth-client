...
// 서버 registrationStep → "마지막 저장 완료" 스텝 번호 매핑
 ...
 * store에 한 번에 rehydrate한 뒤 서버가 알려준 "마지막 저장 완료" 단계의 다음 단계로 점프한다.
...
function useRestoreDuesDraft(clubId: string, accountId: number | null, cardinalNumber: number) {
  ...
  const restoreDraft = async ({ onBasicRestored }: RestoreDuesDraftOptions = {}) => {
    ...
    // registrationStep은 "마지막 저장 완료" 스텝이므로, 이어서 작성은 그다음 스텝부터 시작한다.
