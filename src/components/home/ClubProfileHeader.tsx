...
export function ClubProfileHeader() {
  ...
  return (
    <div className="flex items-center gap-4 px-200 py-300">
      <Avatar type="square">
        {data?.club.profileImageUrl ? (
          <AvatarImage width={64} height={64} alt="clubImage" src={data.club.profileImageUrl} />
        ) : (
          <AvatarFallback>
      ...
      <div className="flex w-[172px] flex-col justify-center gap-[2px]">
        <p className="text-text-strong typo-sub1">{data?.club.name}</p>
        <p className="text-text-normal typo-body2">{data?.club.description}</p>
      </div>
    </div>
  );
}
