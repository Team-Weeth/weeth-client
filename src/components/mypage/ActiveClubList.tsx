...
function ActiveClubList({ clubs = [], clubId, className, ...props }: ActiveClubListProps) {
  return (
    ...
    >
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fill, 182px)' }}>
        {clubs.map((club) => (
...
function ActiveClubCard({ club }: ActiveClubCardProps) {
  return (
    <div className="bg-container-neutral-alternative flex flex-col gap-2 rounded-lg p-450">
      ...
      <div className="flex flex-col">
        ...
        {club.description && (
          <span className="typo-body1 text-text-alternative line-clamp-1">{club.description}</span>
        )}
...
function ProfileManageCard({ clubId }: { clubId: string }) {
  return (
    <Link
      ...
      className="bg-container-neutral-alternative flex h-[220px] flex-col items-center justify-center gap-2 rounded-lg p-450"
    >
