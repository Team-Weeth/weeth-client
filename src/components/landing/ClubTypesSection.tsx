...
function ClubTypesSection({ className }: ClubTypesSectionProps) {
  return (
    <section
      className={cn(
        'flex w-full flex-col items-center gap-[60px] bg-[#F2C6F0] pt-[203px] pb-[204px]',
        className,
      )}
    >
      <div className="flex flex-col gap-[14px] text-center text-[#1E2021]">
        <h2 className="text-center text-[40px] leading-[48px] font-bold tracking-[-0.005em]">
          활동이 달라도, 우리는 함께합니다
        </h2>
        <p className="text-center text-[24px] leading-[30px] font-semibold tracking-[-0.005em]">
      ...
      <div className="flex w-full">
        {CLUB_TYPES.map(({ label, image }) => (
          <div key={label} className="flex flex-1 flex-col gap-[15px]">
            <span className="typo-sub2 text-text-alternative">{label}</span>
