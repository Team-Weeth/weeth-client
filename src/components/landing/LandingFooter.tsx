...
import { FOOTER_MENUS } from '@/constants/landing';
...
function LandingFooter({ className }: LandingFooterProps) {
  return (
    <footer className={cn('flex w-full flex-col items-center bg-[#EEF2F5] px-[160px]', className)}>
        ...
        <div className="flex gap-200">
          {FOOTER_MENUS.map(({ title, items }) => (
            <div key={title} className="flex w-[160px] flex-col gap-200">
              ...
              {items.map((item) => (
                <Link key={item} href="#" className="typo-body2 text-[#909599]">
                  {item}
                </Link>
              ))}
            </div>
