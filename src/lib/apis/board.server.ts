...
export const boardServerApi = {
  ...
  getPostById: (clubId: string, postId: number) =>
    apiServer.get<ApiResponse<PostDetail>>(`${API_BASE_PATH}/clubs/${clubId}/boards/posts/${postId}`),

  /** 공지 읽음 처리 (Server Action) */
  readAllNotices: (clubId: string, boardId: number) =>
    apiServer.post<ApiResponse<void>>(`${API_BASE_PATH}/clubs/${clubId}/boards/${boardId}/notices/read-all`),
};
